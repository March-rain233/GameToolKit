        ...
        where TNode : BaseNode
    {
        ...
        public override bool OnSelectEntry(SearchTreeEntry SearchTreeEntry, SearchWindowContext context)
        {
            ...
            if (type.IsGenericTypeDefinition)
            {
                ...
                var dialog = EditorWindow.CreateWindow<GenericSelectorEditorWindow>();
                dialog.TypeDatas = new GenericSelectorEditorWindow.TypeData[args.Length];
                for(int i = 0; i < args.Length; i++)
                {
                    dialog.TypeDatas[i].Name = args[i].Name;
                    dialog.TypeDatas[i].ValidTypes = GenericSelectorEditorWindow.TypeData.
